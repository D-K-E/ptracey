...
public:
  ...
  bvh_node(const hittable_list &list, double time0,
           double time1)
...
bvh_node::bvh_node(
    const std::vector<shared_ptr<hittable>> &src_objects,
    size_t start, size_t end, double time0, double time1) {
  auto objects = src_objects; // Create a modifiable array
                              // of the source scene objects
  ...
  if (object_span == 1) {
    ...
    left = make_shared<bvh_node>(objects, start, mid, time0,
                                 time1);
    right = make_shared<bvh_node>(objects, mid, end, time0,
                                  time1);
  }
